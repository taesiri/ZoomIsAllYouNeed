...
te_transforms_inc = transforms.Compose(
...
def build_model(args):
    ...
    if hasattr(args, "prior_strength"):
        if not args.prior_strength is None and args.prior_strength >= 0:
            ...
            nn.BatchNorm2d.prior = float(args.prior_strength) / float(
                args.prior_strength + 1
            )
...
def prepare_test_data(args, use_transforms=True):
    if args.corruption in common_corruptions:
        ...
        validdir = os.path.join(
            args.dataroot, "imagenet-c", args.corruption, str(args.level)
        )
        ...
        teset = datasets.ImageFolder(validdir, te_transforms_local)
    elif args.corruption == "imagenet1k_full":
        te_transforms_local = te_transforms if use_transforms else None
        validdir = os.path.join(args.dataroot, "imagenet1k_full")
        teset = datasets.ImageFolder(validdir, te_transforms_local)
    elif args.corruption == "imagenet-sketch":
        te_transforms_local = te_transforms if use_transforms else None
        validdir = os.path.join(args.dataroot, "imagenet-sketch")
        teset = datasets.ImageFolder(validdir, te_transforms_local)
    elif args.corruption == "objectnet":
        te_transforms_local = te_transforms if use_transforms else None
        validdir = os.path.join(args.dataroot, "objectnet")
        teset = datasets.ImageFolder(validdir, te_transforms_local)
